...
- Workflow order is fixed: sync GitButler base -> inspect GitButler state -> select ship target -> simplify -> test/CI -> commit -> push -> PR.
...
- First, update the GitButler base using the native flow: run `but status --upstream`, then `but pull --check`; if upstream work exists and the check passes, run `but pull --status-after` so active branches are rebased onto the latest target branch/base. If conflicts appear, resolve them with `but resolve <commit-id>`, edit conflicted files, verify with `but resolve status`, then `but resolve finish` before continuing.
